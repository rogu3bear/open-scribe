...
|---|---|---|---|---:|---|---|
| TD-002 | Failed-session recovery | Runtime failures stop capture and clear controller references, but a prepared/open partial session has no explicit finalization or user-visible recovery path. | Durable residue may exist without a complete recovery and playback experience. | P0 | Native/durability owners | Explicit failure journal transition plus relaunch/discover/recover/play tests. |
...
| Missing partial-session lifecycle | `LiveMicrophoneRecordingController.fail` | Capture stops and references clear, but Rust has no explicit failed/interrupted session transition exposed to the operator. | P0 | Native/durability owners | Model close/recover ownership and test every transition. |
...
| `LiveMicrophoneRecordingController` | Sprout | Coordinate the explicit menu-bar capture path without moving frame-rate media across UniFFI. | Admitted bounded slice; recovery remains open | Native runtime owner |
