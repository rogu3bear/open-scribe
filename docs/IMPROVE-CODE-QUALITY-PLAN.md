...
- **Current focus:** close the gap from the short live-microphone proof to the North Star's first observable success: explicit source truth plus forced-termination recovery and playback, with interruption handling as supporting infrastructure.
...
|---|---|---:|---|
| 1. Safety Net | in-progress | 2026-08-27 | Controller failures now report durable typed interruption; restart classification and one short real capture passed. Forced-termination/playable-recovery proof remains absent. |
...
| 4. Correctness | in-progress | 2026-08-27 | Failure cleanup and durable interruption are tested; operator-visible playable recovery semantics remain open. |
...
| Preserve explicit retry and cleanup behavior. | Permission denial remains pre-session; post-preparation failures durably report typed interruption before clearing live controller references. | A recovery design that changes partial-session ownership. |
...
- [x] **P0 — Native/durability owners — interruption-state slice:** define and test typed post-preparation interruption, journal-before-projection ordering, restart discovery/repair, media preservation, and truthful non-Recording UI behavior.
...
- [ ] **P0 — Durability owner — before source expansion or transcription:** demonstrate forced-termination discovery and playable recovery without relying on transcription.
- [ ] **P0 — Product/native owner — before a system-audio adapter:** select one application/system-audio capture mode and define required-source identity, authority, and loss behavior.
